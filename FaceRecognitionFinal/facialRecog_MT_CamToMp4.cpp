...
void process_face(const vitis::ai::FaceDetectRecogFixedResult &result,
    ...
    std::string similarity_text = (mapped_score > 0.50f ? "Abheek" : "Unknown") + std::string(": ") + std::to_string(mapped_score);
...
int main(int argc, char *argv[]) {
   ...
   std::string output_file = "output2.avi";
    std::string gstreamer_pipeline = "appsrc ! videoconvert ! avimux ! filesink location=" + output_file;

    // Create VideoWriter with GStreamer pipeline
    cv::VideoWriter video_writer;
    video_writer.open(gstreamer_pipeline, cv::CAP_GSTREAMER, 0, 30.0, cv::Size(320, 320), true);
